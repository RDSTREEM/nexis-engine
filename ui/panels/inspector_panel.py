...
class Vec3Widget(QWidget):
    def __init__(self, label: str, value: np.ndarray, callback, parent=None):
        ...
        for i, axis in enumerate("XYZ"):
            sb = QDoubleSpinBox()
            sb.setRange(-99999, 99999); sb.setDecimals(3)
            sb.setSingleStep(0.1); sb.setValue(float(value[i]))
            sb.setPrefix(f"{axis} ")
            sb.valueChanged.connect(lambda v, i=i: self._changed(i, v))
            row.addWidget(sb); self._spins.append(sb)
    ...
    def refresh(self, value: np.ndarray):
        for i, sb in enumerate(self._spins):
            sb.blockSignals(True); sb.setValue(float(value[i])); sb.blockSignals(False)
...
class InspectorPanel(QDockWidget):
    def __init__(self, app, parent=None):
        ...
        self._layout.setAlignment(Qt.AlignTop); self._layout.setSpacing(4)
    ...
    def show_entity(self, entity) -> None:
        self._entity = entity; self._rebuild()

    def clear(self) -> None:
        self._entity = None; self._rebuild()
    ...
    def _build_header(self):
        e   = self._entity
        row = QHBoxLayout()
        cb  = QCheckBox(); cb.setChecked(e.enabled)
        cb.toggled.connect(lambda v: setattr(e, "enabled", v))
        row.addWidget(cb)
        ne = QLineEdit(e.name)
        ne.textChanged.connect(lambda v: (setattr(e, "name", v),
    ...
    def _build_transform(self):
        ...
        def _mk(label, arr):
            def cb(idx, val): arr[idx] = val; t._dirty = True
    ...
    def _build_component(self, comp):
        ...
        hrow = QHBoxLayout()
        cb   = QCheckBox(); cb.setChecked(comp.enabled)
        ...
        if isinstance(comp, MeshRenderer):
            ...
            # colour picker
            btn = self._color_btn(comp.color,
                                  lambda r,g,b,a: comp.set_color(r,g,b,a))
            ...
            fx.toggled.connect(lambda v: setattr(comp, "flip_x", v) or setattr(comp, "_vao", None))
            fy = QCheckBox(); fy.setChecked(comp.flip_y)
            fy.toggled.connect(lambda v: setattr(comp, "flip_y", v) or setattr(comp, "_vao", None))
            form.addRow("Flip X", fx)
            form.addRow("Flip Y", fy)

        elif isinstance(comp, CameraComponent):
            pc = QComboBox(); pc.addItems(["perspective","orthographic"])
            ...
            form.addRow("FOV", fov)
            near = QDoubleSpinBox(); near.setRange(0.001,100); near.setDecimals(3)
    ...
    def _color_btn(self, color_arr, on_change):
        ...
        def pick():
            ...
            if dlg.exec():
                ...
                btn.setStyleSheet(
                    f"background:rgb({q.red()},{q.green()},{q.blue()});")
    ...
    def _material_rows(self, form, mat):
        btn = self._color_btn(
            mat.color,
            lambda r,g,b,a: mat.set_color(r,g,b,a))
        form.addRow("Color", btn)
        amb = QDoubleSpinBox(); amb.setRange(0,1); amb.setSingleStep(0.05)
    ...
    def _remove(self, comp):
        if self._entity:
            self._entity.remove_component(comp); self._rebuild()
