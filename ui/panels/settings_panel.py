...
DEFAULT_SETTINGS = {
    ...
    "tags": ["Untagged", "Player", "Enemy", "Ground", "Trigger", "UI", "Camera"],
...
class SettingsPanel(QDialog):
    ...
    def _build_physics_tab(self) -> QWidget:
        ...
        ts = QDoubleSpinBox()
        ts.setRange(0.001, 0.1)
        ts.setDecimals(4)
        ts.setValue(p["fixed_timestep"])
    ...
    def _build_input_tab(self) -> QWidget:
        ...
        for axis, bindings in inp.items():
            ...
            pos.textChanged.connect(lambda v, a=axis: inp[a].update({"positive": v}))
            neg.textChanged.connect(lambda v, a=axis: inp[a].update({"negative": v}))
            row = QHBoxLayout()
    ...
    def _add_axis(self, frm, inp):
        ...
        if ok and name.strip() and name not in inp:
            ...
            pos = QLineEdit()
            neg = QLineEdit()
            pos.textChanged.connect(lambda v, a=name: inp[a].update({"positive": v}))
            neg.textChanged.connect(lambda v, a=name: inp[a].update({"negative": v}))
            row = QHBoxLayout()
            row.addWidget(QLabel("+"))
            row.addWidget(pos)
            row.addWidget(QLabel("-"))
    ...
    def _save(self) -> None:
        ...
        if root:
            p = root / SETTINGS_FILE
            p.write_text(json.dumps(self._settings, indent=2), encoding="utf-8")
