"""
gizmos.py
Translate / rotate / scale gizmo handles for 2D and 3D.
Rendered via DebugDraw lines. Mouse drag moves the selected entity.

Usage in viewport:
    gizmo.set_entity(selector.selected_entity)
    gizmo.draw(debug_draw, view, proj, viewport_w, viewport_h)
    # on mouse press:
    gizmo.on_mouse_press(mx, my, view, proj, w, h)
    # on mouse move:
    gizmo.on_mouse_move(mx, my, view, proj, w, h)
    # on mouse release:
    gizmo.on_mouse_release()
"""
...
def _project_to_screen(world_pos: np.ndarray,
                        view: np.ndarray,
                        proj: np.ndarray,
                        vw: int, vh: int) -> Optional[np.ndarray]:
...
class Gizmo:
    ...
    def _draw_translate(self, dd, o, s):
        ...
        dd.line(o + np.array([s,    0, 0]),
                o + np.array([s-.2, .1, 0]), _HL if ax else _X)
        dd.line(o + np.array([s,    0, 0]),
                o + np.array([s-.2,-.1, 0]), _HL if ax else _X)
        # Y arrow
        dd.line(o, o + np.array([0, s, 0]), _HL if ay else _Y)
        dd.line(o + np.array([0,  s,   0]),
                o + np.array([.1, s-.2, 0]), _HL if ay else _Y)
        dd.line(o + np.array([0,  s,   0]),
                o + np.array([-.1,s-.2, 0]), _HL if ay else _Y)
        # Z arrow (3D only)
        if not self._is_2d:
            dd.line(o, o + np.array([0, 0, s]), _HL if az else _Z)
            dd.line(o + np.array([0, 0, s]),
                    o + np.array([.1, 0, s-.2]), _HL if az else _Z)
            dd.line(o + np.array([0, 0, s]),
                    o + np.array([0, .1, s-.2]), _HL if az else _Z)
        # XY plane square (2D)
        if self._is_2d:
            q = s * 0.3
            pts = [o + np.array([0,0,0]),
                   o + np.array([q,0,0]),
                   o + np.array([q,q,0]),
                   o + np.array([0,q,0])]
    ...
    def _draw_rotate_3d(self, dd, o, s):
        ...
        for axis, col in ((0,_X),(1,_Y),(2,_Z)):
            ...
            for i in range(segs+1):
                a = 2*math.pi*i/segs
                if axis == 0:   pt = o+np.array([0,   math.cos(a)*s, math.sin(a)*s])
                elif axis == 1: pt = o+np.array([math.cos(a)*s, 0,   math.sin(a)*s])
                else:           pt = o+np.array([math.cos(a)*s, math.sin(a)*s, 0  ])
                if prev is not None:
                    dd.line(prev, pt,
                            _HL if self._active_axis==axis else col)
                prev = pt

    def _draw_rotate_2d(self, dd, o, s):
        dd.wire_sphere(o, s, _HL if self._active_axis==AXIS_Z else _Z,
                       segments=32)
    ...
    def on_mouse_press(self, mx, my,
        ...
        self._drag_start_angle = math.atan2(
            self._drag_start_world[1] - o[1],
            self._drag_start_world[0] - o[0])
        return True
    ...
    def on_mouse_move(self, mx, my,
        ...
        if self.mode == TRANSLATE:
            ...
            sign  = 1.0 if (delta[self._active_axis] >= 0
                            if self._active_axis < 3 else True) else -1.0
