...
These are what appear in the "Create Entity" dialog — no blank entities.
"""
...
def _make(scene: "Scene", name: str) -> "Entity":
    from core.entity import Entity
    e = Entity(name=name, scene=scene)
    return e
...
def empty_entity(scene, name="Empty") -> "Entity":
    """Bare entity — just a Transform."""
    return _make(scene, name)


def group_entity(scene, name="Group") -> "Entity":
    """
    Empty entity used as a folder/group.
    Children are added via entity.add_child().
    """
    e = _make(scene, name)
    e.tags.append("group")
    return e
...
# ------------------------------------------------------------------
# Registry — used by the Create Entity dialog
# ------------------------------------------------------------------

TEMPLATES = {
    # label: (factory_fn, category, description)
    ...
    "Mesh (Cube)":        (lambda s: mesh_entity(s, "Mesh", "cube"),      "3D", "Cube mesh with default material"),
    "Mesh (Sphere)":      (lambda s: mesh_entity(s, "Mesh", "sphere"),    "3D", "Sphere mesh"),
    "Mesh (Plane)":       (lambda s: mesh_entity(s, "Mesh", "plane"),     "3D", "Plane mesh"),
    "Mesh (Cylinder)":    (lambda s: mesh_entity(s, "Mesh", "cylinder"),  "3D", "Cylinder mesh"),
    ...
    "Trigger":            (trigger_entity,          "Physics",  "Trigger collider (no physics response)"),
    "Audio Source":       (audio_entity,            "Audio",    "Audio source entity"),
    "Scripted Entity":    (scripted_entity,         "Script",   "Entity with a ScriptComponent attached"),
}